...
_.each = function(collection, func){
    ...
    if (Array.isArray(collection)){ // determine if Array.isArray(collection) is truthy
...
_.every = function(collection, func) {
    ...
    if (func === undefined) {
      ...
      _.each(collection, function(value, key, collection) {
        if (func(value, key, collection) === false) {
          result = false;
        }
      });
    }
...
_.some = function(collection, func) {
    ...
    if (Array.isArray(collection)) {
      for (let i = 0; i < collection.length; i++) {
        if (Boolean(func(collection[i], i, collection))) {
          return true;
        }
      }
    } else {
      for (let key in collection) {
        if (Boolean(func(collection[key], key, collection))) {
          return true;
        }
      }
    }
...
_.extend = function(objectOne, ...objs) {
    return Object.assign(objectOne, ...objs);
  }
